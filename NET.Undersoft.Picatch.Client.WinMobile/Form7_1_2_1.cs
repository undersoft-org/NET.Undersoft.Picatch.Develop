...
namespace SmartDeviceApplication2
{
	...
	/// </summary>
	public class Form7_1_2_1 : System.Windows.Forms.Form
    {
        public Label label1;
		...
		public void senddoc()
		{
			...
			SqlCeConnection cn = new SqlCeConnection(connectionString);
			cn.Open();
			SqlCeDataAdapter da = new SqlCeDataAdapter("SELECT * FROM dane", cn);
			DataTable table = new DataTable();
			da.Fill(table);
			string index = table.Rows[rownum][0].ToString();
			SqlCeCommand cmdh = cn.CreateCommand();
			cmdh.CommandText = "SELECT kod, bad_cena, bad_stan, cenapolka, zliczono FROM dane WHERE bad_cena = 1 or bad_stan = 1";
			cmdh.Prepare();
			SqlCeDataReader drh = cmdh.ExecuteReader();
			while (drh.Read())
			{
				sw.WriteLine(drh.GetString(0)+";"+Convert.ToString(drh.GetBoolean(1))+";"+Convert.ToString(drh.GetBoolean(2))+";"+Convert.ToString(drh.GetDecimal(3))+";"+Convert.ToString(drh.GetDecimal(4)));
			}
		...
		private void InitializeComponent()
		{
            ...
            this.label1.Text = "CZY CHCESZ WYS£AÆ DANE SPRAWDZARKI?";
