	...
	public class Form1 : System.Windows.Forms.Form
	{
		...
		public void Openbaza()
		{
			...
			if (File.Exists(fileName) == false)
			{
				...
				cn.Open();
				SqlCeCommand cmd = new SqlCeCommand("CREATE TABLE dane (typ nvarchar (7), kod nvarchar (15) not null primary key, nazwa nvarchar(40), stan nvarchar(10), cenazk nvarchar(10), cenasp nvarchar(10), vat nvarchar(5), devstat nvarchar(10), bad_cena bit, bad_stan bit, cenapolka numeric(6,3), zliczono numeric(6,3), datazmian datetime, cenahurt nvarchar(10), cenaoryg nvarchar(10))", cn);
				cmd.ExecuteNonQuery();
				cmd = new SqlCeCommand("CREATE INDEX kod ON dane (kod)", cn);
				cmd.ExecuteNonQuery();
				cmd = new SqlCeCommand("CREATE TABLE bufor (id int identity not null primary key, dokid int not null, kod nvarchar(15) not null, nazwa nvarchar (40), cenazk nvarchar(10), ilosc numeric(10,3), stan nvarchar(10), cenasp nvarchar(10), vat nvarchar(10))", cn);
				cmd.ExecuteNonQuery();
				cmd = new SqlCeCommand("CREATE INDEX dokid ON bufor (dokid)", cn);
				cmd.ExecuteNonQuery();
				cmd = new SqlCeCommand("CREATE INDEX kod ON bufor (kod)", cn);
				cmd.ExecuteNonQuery();
				...
				cmd.ExecuteNonQuery();
				cmd = new SqlCeCommand("CREATE TABLE opcje (id int identity not null primary key, transfer nvarchar (20), com nvarchar(20), ip nvarchar(20), ufile nvarchar (120), dfile nvarchar(120), bdll nvarchar(50), bflag bit, ipflag bit, port int, skaner nvarchar(120), licence nvarchar(40), devstat nvarchar(20), impdata datetime)", cn);
				cmd.ExecuteNonQuery();
				cmd = new SqlCeCommand("INSERT INTO opcje (transfer, com, ip, ufile, dfile, bdll, bflag, ipflag, port, skaner, licence, impdata) VALUES ('bluetooth', 'COM5', '10.0.0.101', '\\Inwent.imp', '\\Inwent.exp', 'MSStack', 0, 1, 8790, '\\Program Files\\Power Tools\\ScanWedge.exe', '0000000000', '1999-01-01')", cn);
				cmd.ExecuteNonQuery();
				...
				cmd.ExecuteNonQuery();
				cmd = new SqlCeCommand("CREATE INDEX MagId ON magazyn (MagId)", cn);
				cmd.ExecuteNonQuery();
				cmd = new SqlCeCommand("CREATE TABLE stany (id int identity not null primary key, kod nvarchar (15) not null, stan nvarchar(20), MagId int not null, Nazwa nvarchar(20), datazmian datetime)", cn);
				cmd.ExecuteNonQuery();
				cmd = new SqlCeCommand("CREATE INDEX kod ON stany (kod)", cn);
				cmd.ExecuteNonQuery();
		...
		{
			Form71 frm71 = new Form71(licflag);
			frm71.Show();
		}
