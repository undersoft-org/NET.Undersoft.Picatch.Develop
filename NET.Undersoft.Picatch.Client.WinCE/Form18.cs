...
namespace Undersoft.Picatch
{
	...
	public class Form18 : System.Windows.Forms.Form
	{
		...
		int czydodac;
		int ebid;
		private Undersoft.Picatch.Form17 form17;
		int rownum;
		public Form18(string dokus, string kod, string nazwik, string cenik, string ceniksp, string vacik, string wymagane, string zliczone, string statusik, string opak, string jedn, string asorcik, Undersoft.Picatch.Form17 form17a, int rownumber, int addflag, string ilosc, int id)
		{
			kodzik = kod;
			index = dokus;
			rownum = rownumber;
		...
		private void WriteLine ()
		{
			if (czydodac == 1)
			{
				...
				cmdb.CommandText = "INSERT INTO fedibody (NrDok, Nazwa, kod, Vat, Jm, Asortyment, Ilosc, Cena, IleWOpak, CenaSp, ebid, Wymagane) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
				cmdb.Parameters.Add("@1", SqlDbType.NVarChar, 30);
				...
				cmdb.Parameters.Add("@14", SqlDbType.Int, 10);
				cmdb.Parameters.Add("@15", SqlDbType.NVarChar, 10);
				
				cmdb.Prepare();

				cmdb.Parameters["@1"].Value = index;
				cmdb.Parameters["@2"].Value = nazwa_t.Text;
				cmdb.Parameters["@3"].Value = kod_t.Text;
				cmdb.Parameters["@4"].Value = vat_t.Text;
				cmdb.Parameters["@5"].Value = jm_t.Text;
				cmdb.Parameters["@6"].Value = asorcik1;
				cmdb.Parameters["@9"].Value = ilosc_t.Text;
				cmdb.Parameters["@10"].Value = cena_t.Text;
				cmdb.Parameters["@12"].Value = wopak_t.Text;
				cmdb.Parameters["@13"].Value = cenasp_t.Text;
				cmdb.Parameters["@14"].Value = ebid;
				cmdb.Parameters["@15"].Value = wymagane_t.Text;
			...
			else if (czydodac == 0)
			{
				...
				SqlCeCommand cmdb = cn.CreateCommand();
				cmdb.CommandText = "INSERT INTO edibody (NrDok, Nazwa, kod, Vat, Jm, Asortyment, Ilosc, Cena, IleWOpak, CenaSp, status, complete) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
				...
				cmdb.Parameters["@12"].Value = wopak_t.Text;
				cmdb.Parameters["@13"].Value = cenasp_t.Text;
				cmdb.Parameters["@14"].Value = "Nowy";
				cmdb.Parameters["@15"].Value = byte.Parse("0");
				cmdb.ExecuteNonQuery();			
			...
			else if (czydodac == 2)
			{
				...
				cmdb.Parameters.Add("@1", SqlDbType.NVarChar, 20);
				cmdb.Parameters.Add("@2", SqlDbType.Int, 10);	
				

				cmdb.Prepare();

				cmdb.Parameters["@1"].Value = ilosc_t.Text;
				cmdb.Parameters["@2"].Value = ebid;
		...
		private void InitializeComponent()
		{
			...
			// wymagane_t
			// 
			this.wymagane_t.BackColor = System.Drawing.Color.GreenYellow;
			this.wymagane_t.Font = new System.Drawing.Font("Lucida Sans Unicode", 8.25F, System.Drawing.FontStyle.Regular);
			this.wymagane_t.ForeColor = System.Drawing.Color.Black;
